...
SURVEYS=['desi','eboss','cosmos','eboss_V2']
...
def draw_points_eboss(radec,unique_ids,obj='star',seed=1,
	...
	Returns:
		Nothing, but write a fits_table containing the unique id, ra, dec
			and color + redshift + morphology info for each source
	"""
    print('entered draw_points eboss 01')
    ndraws= len(unique_ids)
    random_state= np.random.RandomState(seed)
    ra,dec= get_radec(radec,ndraws=ndraws,random_state=random_state)
    # Load samples
    ...
    dr3dp2_exp= pd.read_csv(os.path.join(mog_param_dir(),
                            'vvds_dp2_exp.csv')) # in my google drive
    ...
    dr3dp2_exp_trimmed = pd.read_csv(os.path.join(mog_param_dir(),
                                  'vvds_dp2_exp_trim.csv'))
    dr3dp2_dev_trimmed = pd.read_csv(os.path.join(mog_param_dir(),
                                 'vvds_dp2_dev_trim.csv'))     
    ...
    trees= dict(dr3dp2_exp= spatial.KDTree(dr3dp2_exp['redshift'].values.reshape(-1,1)),
                ...
                dr3dp2_exp_trimmed = spatial.KDTree(dr3dp2_exp_trimmed['redshift'].values.reshape(-1,1)),
                dr3dp2_dev_trimmed = spatial.KDTree(dr3dp2_dev_trimmed['redshift'].values.reshape(-1,1)))
    ...
    keep= (~inBox['dr3dp2_both']) & (T.type == 'EXP')
    print('exp out:'+str(keep.sum()))
   
    #_,i_df= trees['dr3dp2_exp_trimmed'].query(T.redshift[keep].reshape(-1,1))
    i_df=i_both[keep]
    for col in mag_shapes:
        d[col][keep]= dr3dp2_both[col].iloc[i_df]
    d['id'][keep]= dr3dp2_both['tractor_id'].iloc[i_df]
    d['nn_redshift'][keep]= dr3dp2_both['redshift'].iloc[i_df]

    keep= (~inBox['dr3dp2_both']) & (T.type == 'DEV')
    print('dev out:'+str(keep.sum()))
    #_,i_df= trees['dr3dp2_dev_trimmed'].query(T.redshift[keep].reshape(-1,1))
    i_df=i_both[keep]
    for col in mag_shapes:
        d[col][keep]= dr3dp2_both[col].iloc[i_df]
    d['id'][keep]= dr3dp2_both['tractor_id'].iloc[i_df]
...
def get_parser():
    ...
    parser.add_argument('--survey', type=str, choices=['desi','eboss','cosmos','eboss_V2'], default=None, required=True)
