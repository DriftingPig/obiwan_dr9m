...
def draw_points(radec,unique_ids,obj='star',seed=1,
    ...
    if survey == 'desi':
        ...
        return draw_points_eboss(radec,unique_ids,obj=obj,seed=seed,
                                 outdir=outdir,startid=startid)
    elif survey == 'eboss_V2':
         return draw_points_eboss_V2(radec,unique_ids,obj=obj,seed=seed,
                                      outdir=outdir,startid=startid)    
...
def draw_points_eboss_V2(radec, unique_ids, obj='elg',seed=1,outdir='./',startid=1):
        """
        Args:
                radec: dict with keys ra1,ra2,dec1,dec2
                        the ra,dec limits for the sample
        ...
        sample_elg_dr3_like = fits.getdata(os.path.join(topdir+'sample_dr3_elg_like_updated.fits'))
        T.set('ba', np.random.uniform(0.2,1.,size=ndraws))
        T.set('pa', np.random.uniform(0.,180.,size=ndraws))
        ids = random_state.randint(low=0,high=len(sample_elg_dr3_like),size=ndraws)
        T.set('g',sample_elg_dr3_like['g'][ids])
        T.set('r',sample_elg_dr3_like['r'][ids])
        T.set('z',sample_elg_dr3_like['z'][ids])
        T.set('n',sample_elg_dr3_like['n'][ids])
        T.set('r_half',sample_elg_dr3_like['r_half'][ids])
        T.set('id_sample',sample_elg_dr3_like['id_sample'][ids])
        T.set('redshift',-1*np.ones(ndraws))
        fn = '/global/cscratch1/sd/huikong/obiwan_Aug/repos_for_docker/obiwan_out/eboss_elg/dr3_elg_like/dr3_elg_like_seed'+str(seed)+'.fits'
        T.writeto(fn)
        print('Wrote %s' % fn)
...
def get_parser():
    ...
    parser.add_argument('--survey', type=str, choices=['desi','eboss','eboss_V2','cosmos'], default=None, required=True)
